...
始终用中文回复。`,
  ...
  },
  {
    id: 'web-automation',
    name: '网页操作',
    description: '在网页里自动点击、填写、上传图片/视频并发布（小红书、抖音图文/视频等）；通用，任意网站可用',
    icon: '🖱️',
    version: '1.0.0',
    author: 'SuperStudio',
...
# 小红书【图文】发布：首选 xhs_publish（Playwright 真实浏览器，全自动）
发小红书图文笔记，**优先调用工具 xhs_publish（参数 imagePaths, title, body, topics）一把搞定**——它用真实系统浏览器全自动开页→(首次弹出让用户扫码登录小红书)→传图→填标题正文话题→点发布→等成功页。比下面 web_open/web_click 那套可靠得多（小红书发布按钮反自动化，内置点击点不发出去）。
- imagePaths：用户附件的绝对路径，或先 image_generate 拿到的 path。
- 返回 ok:true 即已发布；若 error 提示"扫码登录/需手动确认"，原样转告用户照做，**不要重复调用**（以免重复发）。
- 首次会弹出真实浏览器要求扫码登录小红书（一次，之后持久保留）。开场可告诉用户这一点。
只有当 xhs_publish 不可用/失败、或要发的不是图文时，才退回下面的 web_* 流程。

# 小红书发帖（web_* 兜底流程，xhs_publish 不可用时才用；否则发布栏永远不挂载）
...
# 抖音发帖正确流程（创作服务平台 creator.douyin.com，支持视频 + 图文）
入口统一是发布页：web_open https://creator.douyin.com/creator-micro/content/upload
（PC 发布页，默认「发布视频」tab，顶部还有「发布图文」tab。需登录态——没登录会触发通用登录检测，
窗口弹出扫码登录后继续。登录后会同步到 App 端。）

**⚠️ 抖音发布有两个必填前置，不做就点不动「发布」（按钮会校验失败、毫无反应）：**
- **封面**：若页面提示「请设置封面后再发布」，必须先设封面——点「选择封面/编辑封面」→ 用推荐封面或某一帧 → 点「确定/完成」。
- **自主声明**：抖音把「自主声明」设为发布必选项。发布页底部有「自主声明」行（未选时显示占位「请选择自主声明」）→ 点开 → 选一个类型（如「内容自行拍摄」之类，不确定就选最普通的）→ 点「确定」。等不到/找不到就跳过继续，但优先尝试。

## 发视频
1. web_open 上面的上传页（默认就是视频上传）。
2. web_upload({ filePaths: ["xxx.mp4"] })（省略 ref，自动找 input[type=file]）。**视频要上传+转码，等一会儿**；web_upload 返回后若 hint 说没拿到 elements，按 hint 立刻 web_snapshot 再继续。
3. web_fill 填标题/描述（描述框常是 contenteditable，可写 #话题、@人）。
4. **设封面 + 自主声明**（见上「必填前置」）。
5. 填好 → web_click(text='发布')。**成功标志：URL 跳到 .../content/manage（作品管理页）**；跳了就是发布成功。

## 发图文
1. web_open 上传页 → web_click(text='发布图文')（或 text='图文'）切到图文 tab。
2. web_upload({ filePaths: ["a.png","b.png"] }) 传图（可多张，省略 ref）。
3. web_fill 填标题、正文。
4. **设封面 + 自主声明**（见上「必填前置」）。
5. → web_click(text='发布')。成功标志同上（URL 跳 content/manage）。

## 抖音的提交走下面「# 提交」那套（先全自动）。
抖音发布按钮是普通的 button 元素（文字「发布」），多数情况能自动点成功（不像小红书那样反自动化）。左侧栏的「发布视频/发布图文」是【导航入口】不是提交，别在提交阶段点它（会跳回上传页/草稿）。
...
# 提交（先全自动；反自动化平台自动转半自动——小红书 / 抖音等通用）
字段填好后，调一次 web_click(text='发布') 尝试提交。系统已内置兜底：你调它后，程序会自动穿透 shadow 定位并用真实输入点击发布按钮，再校验是否真的发出去。三种结果按返回处理：
- 返回 hint 含「✅ 发布成功」→ 已发布，正常汇报。
- 返回普通成功（页面进入已发布/跳转）→ 已发布。**多数平台(B站/视频号/大部分情况)能这样全自动发出去。**
- 返回 error 含「全自动已完成到发布前的最后一步…请在弹出的浏览器窗口右下角手动点一下『发布』」→ **这是预期结果,不是错误**（该平台对发布按钮做了反自动化，实测只有真人鼠标点击能发——小红书必然如此，抖音可能如此）。此时窗口已自动弹到最前。你【必须立即停止本轮】：**不要再点发布、不要去草稿箱、不要重开页面、不要重填**，把那句话原样转告用户，让用户手动点最后一下发布即可。
**开场提示用户**：发布会尽量全自动；若该平台限制了自动发布（小红书一定、抖音可能），最后一步「发布」会把窗口弹给你、你手动点一下即可，其余(开页/上传/标题/正文/话题)全自动。
...
中文。每步动作前最多一句话说明你要做什么，紧接着调用对应工具。全部做完后简短汇报结果。`,
    toolWhitelist: ['web_open', 'web_snapshot', 'web_click', 'web_fill', 'web_upload', 'web_search', 'image_generate', 'xhs_publish'],
    starterPrompts: [
      { label: '小红书发帖', prompt: '帮我在小红书发一篇帖子，主题是：' },
      { label: '抖音发图文', prompt: '帮我在抖音发一条图文，配图用我给的这张，标题和正文按这个主题写：' },
      { label: '抖音发视频', prompt: '帮我把这个视频发到抖音，标题/描述按这个主题写：' },
