...
// Exact repo doc filenames only — must NOT match agent files like
// `security-penetration-tester.md` (the `^SECURITY` prefix bug dropped all 10
// security/ agents). Anchor on the full `.md` filename.
const isDoc = (b) => /^(README|CONTRIBUTING|SECURITY|QUICKSTART|EXECUTIVE-BRIEF|PULL_REQUEST)\.md$/i.test(b)
...
// Optional Chinese localization cache (id → {name_zh, desc_zh}), produced by the
// translate-agency-agents workflow. When present, the market shows Chinese names
// + descriptions (display_name.zh wins over profession in pack-talent); otherwise
// it falls back to the English name as profession + English blurb.
// Committed (not under gitignored sources/) so the translation work survives a
// fresh clone / re-ingest. id → {name_zh, desc_zh} from translate-agency-agents.
const TRANS_PATH = 'scripts/agency-agents-zh.json'
const trans = {}
try {
  if (fs.existsSync(TRANS_PATH)) for (const x of JSON.parse(fs.readFileSync(TRANS_PATH, 'utf8'))) {
    if (x && x.id) trans[x.id] = x
  }
} catch (e) { console.warn('translations read failed:', e.message) }

let idx = '# Auto-generated by scripts/ingest-agency-agents.mjs — curated subset of\n'
idx += '# msitarzewski/agency-agents (MIT). domains drive dept; zh from _translations.zh.json.\n'
idx += 'experts:\n'
let zhCount = 0
for (const e of experts.sort((a, b) => a.lid.localeCompare(b.lid))) {
  ...
  const tr = trans[e.lid]
  if (tr && tr.name_zh) {
    zhCount++
    idx += `  display_name:\n    zh: ${yamlStr(tr.name_zh)}\n    en: ${yamlStr(e.name)}\n`
    idx += `  description:\n    zh: ${yamlStr(tr.desc_zh || e.desc)}\n`
  } else {
...
console.log(`[ingest] 中文本地化: ${zhCount}/${experts.length}`)
